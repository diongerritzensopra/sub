...
describe('popup integration tests', () => {
  ...
  describe('popup wiring coverage', () => {
    ...
    it('clears a persisted status when dismiss is clicked', async () => {
      ...
      expect(document.getElementById('status-message')?.textContent).toBe('Tijdelijk bericht');

      (document.getElementById('btn-status-dismiss') as HTMLButtonElement).click();
      await flushAsyncWork();
    ...
    it('opens the schedule form from add button when a snapshot is available', async () => {
      ...
      (document.getElementById('btn-add-schedule') as HTMLButtonElement).click();
      await flushAsyncWork();

      expect(document.getElementById('schedule-form-section')?.hidden).toBe(false);
    });

    it('matches SAP timesheet tabs through isTimesheetTab', async () => {
      const {isTimesheetTab} = await import('./popup');

      expect(isTimesheetTab({url: 'https://p10mq7ma.launchpad.cfapps.eu10.hana.ondemand.com/site#timesheet-my'} as chrome.tabs.Tab)).toBe(true);
      expect(isTimesheetTab({url: 'https://example.com'} as chrome.tabs.Tab)).toBe(false);
    ...
    it('hides the schedule form when cancel button is clicked', async () => {
      ...
      (document.getElementById('schedule-form-cancel') as HTMLButtonElement).click();
      await flushAsyncWork();

      expect(document.getElementById('schedule-form-section')?.hidden).toBe(true);
    });
    ...
    it('shows an error when add-schedule is clicked but no snapshot is loaded', async () => {
      ...
      const btn = document.getElementById('btn-add-schedule') as HTMLButtonElement;
      btn.disabled = false;
      btn.click();
      await flushAsyncWork();

      expect(document.getElementById('status-message')?.textContent)
        .toContain('Analyseer eerst de huidige timesheet voordat je een schema toevoegt');
    });
    ...
    it('shows an error when edit-schedule is triggered but no snapshot is loaded', async () => {
      ...
      const editBtn = document.querySelector('.schedule-edit-button') as HTMLButtonElement;
      editBtn.click();
      await flushAsyncWork();

      expect(document.getElementById('status-message')?.textContent)
        .toContain('Analyseer eerst de huidige timesheet voordat je een schema bewerkt');
    });
  });
  ...
  describe('popup utility function coverage', () => {
    it('restoreCachedStatusMessage returns false and clears cache for an invalid timestamp', async () => {
      mockChromeStorageLocalGet.mockImplementation((keys, callback) => {
        const value = keys[0] === STORAGE_KEYS.statusMessageCache
    ...
    it('restoreCachedStatusMessage returns false and clears cache when entry is older than 30 minutes', async () => {
      ...
      const setupMock = () => mockChromeStorageLocalGet.mockImplementation((keys, callback) => {
        const value = keys[0] === STORAGE_KEYS.statusMessageCache
    ...
    it('restoreCachedStatusMessage returns true and displays a fresh cached message', async () => {
      ...
      expect(document.getElementById('status-message')?.textContent).toBe('Vers bericht');
    });
